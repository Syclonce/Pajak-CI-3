    ...
    <table>
        ...
        <tbody>
            <?php foreach ($catakLaporan as $row) : ?>
                <tr>
                    <td><?= $row->no_faktur; ?></td>
                    <td><?= date('y-m-d', strtotime($row->tanggal_pembelian)) ?></td>
                    <td><?= $row->supplier; ?></td>
                    <td><?= $row->nama_barang; ?></td>
                    <td>Rp. <?= number_format($row->harga, 0, ',', '.') ?></td>
                    <td><?= 'Rp ' . number_format($row->pph, 0, ',', '.'); ?></td>
                    <td><?= 'Rp ' . number_format($row->npwp, 0, ',', '.'); ?></td>
                    <td><?= 'Rp ' . number_format($row->harga + $row->pph + $row->npwp, 0, ',', '.'); ?></td>
                    <!-- Tambahkan kolom-kolom lainnya sesuai dengan data Anda -->
                </tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    echo 'Rp ' . number_format($totalPPH, 0, ',', '.');
                    ?>
                </td>
                <td>
                    <?php
                    ...
                    echo 'Rp ' . number_format($totalNPWP, 0, ',', '.');
                    ?>
                </td>
                <td>
                    <?php
                    $totalKeseluruhan = 0;
                    foreach ($catakLaporan as $row) {
                        $totalKeseluruhan += $row->harga + $row->pph + $row->npwp;
                    }
